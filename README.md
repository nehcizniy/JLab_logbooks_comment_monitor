...
This Chrome extension checks any user-selected JLab logbooks on a selectable 5, 10, 15, 30, or 60 minute interval. DTM beam-down events use a separate selectable 1, 5, 10, 15, 30, or 60 minute interval. Each logbook has its own adjustable check range: the latest 1–1000 entries, the latest 1–720 hours, or the latest 1–30 days. The default is 100 entries. HCLOG, HBLOG, and SOLID are included initially, and every added logbook can be turned on or off independently. It monitors new comment permalinks and beam-down event state. It can also notify you when selected people appear as an entry's **Author** or **Entry Maker**.

The extension has four interface modes using the same monitoring engine. **Simple** is the default and keeps common controls on one page. **Large simple** provides the same focused controls with substantially larger type, buttons, fields, and spacing. **Advanced** exposes every per-logbook, delivery, Shift Crew, diagnostic, email, and backup setting. **Large advanced** provides all of those Advanced controls with the larger, roomier presentation. Use the Interface dropdown near the top of the popup to switch instantly. The persistent **Dark mode** switch works with every interface and also styles the update guide. Switching appearance or interface does not reset monitoring baselines, settings, active notifications, or alarm history.
...
Select **Use default** to restore 5-minute logbook and DTM checks, Standard alerts, DTM changes only, no quiet hours, and no active pause. This does not remove logbooks, watched names, schedules, email configuration, monitoring baselines, or alarm history.
...
## Simple, Large simple, Advanced, and Large advanced interfaces

Simple mode has three tabs: **Monitoring**, **DTM**, and **Settings**. Monitoring keeps current status, recent alarms, Shift Crew, and shift summaries together, with one direct button per enabled logbook at the bottom for quick access. Settings contains Extension updates, the automatic check interval, Alarm level and pause controls, Monitored logbooks, Notify for new entries by, and Test my setup. **Large simple** uses the same organization at a wider size with larger text and controls. The DTM tab shows its separate interval, recurring reminders, status, and live-page shortcut. In Advanced and Large advanced, each control remains in its detailed section, with Extension updates and Test my setup under Settings. Adding and opening a logbook remain available. The Interface dropdown sits on the same row as the Interface label and theme button, with no extra panel to open. Detailed ranges, logbook removal, individual alert channels, quiet hours, per-hall Shift Crew change alerts, email setup, health diagnostics, backup tools, and individual tests appear in both **Advanced** and **Large advanced**; Large advanced enlarges those controls for easier reading and use.

Advanced settings continue running when the interface is switched to Simple or Large simple. Switch back to Advanced or Large advanced whenever you need to view or change them. The theme button shows **Dark mode** in the light theme and **Light mode** in the dark theme. The selected interface and theme choices are included in settings exports and restored with the rest of the configuration.

Every collapsible popup section remembers whether the user left it open or closed. **Shift summaries** starts collapsed on a new installation and then follows the user’s saved choice. These layout choices are stored with the extension settings, migrate automatically from earlier versions, and survive normal updates.
...
Open **Settings** in the popup. Use **Track new versions** to enable or disable automatic daily checks. Select **Check now** to refresh the release status manually, **Download update** for the newest release ZIP, **Previous versions** for the complete release list, or **Update or go back** for the full built-in guide.
...
Use the same steps with an older GitHub Release to roll back. Keeping the previous folder makes the first rollback especially quick: restore that folder to the original path and select **Reload** again. After version 2.16.0, the fixed extension ID identifies the extension, so Reload preserves locally stored settings, monitoring history, interface choices, and collapsed/expanded sections. Do not select **Remove** during an ordinary update or rollback, because removing the extension may reset its settings. Older releases may not understand settings introduced later, so exporting a backup first is recommended.
...
The collapsible **Shift Crew** section sits between **Recent alarms** and **Shift summaries**. In Advanced and Large advanced modes, a compact **A / B / C / D** selector sits on the right side of the Shift Crew heading. Move the pointer onto a letter, or keyboard-focus it, to preview that Hall in the floating crew card. Select a letter to add or remove that Hall from the crew list shown in Simple and Large simple modes. Selected letters are highlighted. Expanding Shift Crew continues to show all four Hall URL fields, current-crew results, change-alert options, and schedule links together. Paste supported public JLab shift-schedule URLs into the Hall rows and select **Enter**. A successful Enter saves every Hall URL, checks the schedules, and folds the section. Blank halls remain blank. The extension then refreshes every configured URL once per day, independently of the main logbook-monitor switch. **Check now** also refreshes the configured schedules.

In Simple and Large simple modes, the A/B/C/D selector is hidden and Shift Crew displays the chosen Halls as a normal list. In Advanced modes, hovering over the folded heading displays the floating preview card for the Hall currently under the pointer. Crew details include the current crew, next crew, and next handoff time using JLab local time. Select a Hall link to open its configured schedule. The daily check stores the day's schedule, so the displayed crew changes at the schedule's shift boundaries without downloading the page again. The Simple-mode Hall selection is saved and included in settings backups.
...
The popup's tracking details show the newest entry and comment-monitoring state for each logbook; independent beam-event status appears in the DTM tab. JLab's logbook listing does not publish comment totals, so the popup does not display misleading zero counts. After creating a test comment, click **Check now** and confirm that a notification appears.
...
The extension checks the DTM open-events page directly without opening background tabs. Its alarm is independent of logbook selection and the logbook check interval, with 1, 5, 10, 15, 30, and 60 minute options. The master monitor switch still pauses both kinds of checks. It uses the DTM event ID as the identity and includes the issue title, start time, and elapsed duration in alerts. At the default **Standard** level, it notifies when an event opens, changes to a different event or title, or closes. **Everything** also sends a recurring reminder on every successful DTM check while the event remains open. The dedicated **DTM** tab is available in all four interface modes; it shows the latest DTM check status, controls its interval and recurring reminders, and opens JLab's live DTM Open Events page. Event alerts include **Clear** and **Go to entry**.
