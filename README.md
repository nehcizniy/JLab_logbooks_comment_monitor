...
Simple mode has three tabs: **Monitoring**, **DTM**, and **Settings**. Monitoring keeps current status, recent alarms, Shift Crew, and shift summaries together. Settings contains Extension updates, the automatic check interval, Alarm level and pause controls, Monitored logbooks, Notify for new entries by, and Test my setup. **Large simple** uses the same organization at a wider size with larger text and controls. The DTM tab shows its separate interval, recurring reminders, status, and live-page shortcut. In Advanced and Large advanced, each control remains in its detailed section, with Extension updates and Test my setup under Settings. Adding and opening a logbook remain available. Detailed ranges, logbook removal, individual alert channels, quiet hours, per-hall Shift Crew change alerts, email setup, health diagnostics, backup tools, and individual tests appear in both **Advanced** and **Large advanced**; Large advanced enlarges those controls for easier reading and use.

Advanced settings continue running when the interface is switched to Simple or Large simple. Switch back to Advanced or Large advanced whenever you need to view or change them. The Interface section folds to save space while keeping its theme button available; it shows **Dark mode** in the light theme and **Light mode** in the dark theme. Open Interface to change the layout. The selected interface and theme choices are included in settings exports and restored with the rest of the configuration.
