...
Select **Use recommended defaults** to restore 5-minute logbook and DTM checks, Standard alerts, DTM changes only, no quiet hours, and no active pause. This does not remove logbooks, watched names, schedules, email configuration, monitoring baselines, or alarm history.
...
Simple mode shows the master monitor switch, status, Check now, Clear alerts, five recent alarms, extension updates, the logbook interval, logbook on/off controls, watched names, current Shift Crew, Hall A–D schedule URL fields, shift-summary links, alert level, pause controls, and Test my setup. **Large simple** shows exactly this focused layout at a wider size with larger text and controls. The DTM tab shows its separate interval, recurring reminders, status, and live-page shortcut. Adding and opening a logbook remain available. Detailed ranges, logbook removal, individual alert channels, quiet hours, per-hall Shift Crew change alerts, email setup, health diagnostics, backup tools, and individual tests appear in both **Advanced** and **Large advanced**; Large advanced enlarges those controls for easier reading and use.

Advanced settings continue running when the interface is switched to Simple or Large simple. When any hidden advanced behavior is active, either focused mode shows a short notice and a **Switch to Advanced** button. The selected interface mode is included in settings exports and restored with the rest of the configuration.
